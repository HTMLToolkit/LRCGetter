...
const downloadAllLyrics = async () => {
  ...
  try {
    ...
    let downloadTrackIds = await store.getTrackIds({
      syncedLyricsTracks: !config.skipTracksWithSyncedLyrics,
      plainLyricsTracks: !config.skipTracksWithPlainLyrics,
      instrumentalTracks: !config.skipTracksWithSyncedLyrics && !config.skipTracksWithPlainLyrics,
      noLyricsTracks: true,
    })
    addToQueue(downloadTrackIds)
  } catch (error) {
    toast.error(error?.data?.message || 'An error occurred while preparing downloads')
    console.error(error)
  } finally {
