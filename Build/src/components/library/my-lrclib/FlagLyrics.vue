<template>
  <BaseModal
    ...
    :close-button="!isFlagging"
    @close="emit('close')"
  >
    <div class="flex flex-col items-center">
      <div v-if="!isFlagging">
        <div class="mb-4">
          Do you want to flag the lyrics of the song
          <strong>{{ track.name }} - {{ track.artistName }}</strong
          >?
        </div>

        <label for="flagReason" class="mb-2 text-xs font-bold"
          >Please explain why you want to flag the lyrics:</label
        >
        <textarea
          id="flagReason"
          v-model="flagReason"
      ...
      <div v-else class="mb-4">
        Flagging the lyrics of the song <strong>{{ track.name }} - {{ track.artistName }}</strong
        >...
      </div>
    ...
    <template #footer>
      <div v-if="!isFlagging" class="flex gap-2 justify-center w-full">
        <button class="button button-primary px-8 py-2 rounded-full" @click="flagLyrics">
          Confirm Flag
        </button>
        <button class="button button-normal px-8 py-2 rounded-full" @click="$emit('close')">
          Cancel
        </button>
      </div>
...
const flagLyrics = async () => {
  ...
  try {
  // Flag operation not implemented in web version
    console.warn('Flag lyrics not available in web version')
...
onMounted(() => {
  // Tauri event listener - not available in web version
})
</script>
