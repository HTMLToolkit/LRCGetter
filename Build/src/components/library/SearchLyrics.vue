...
import { normalizeLrclibLyrics, parseLrcLines, serializeLyricsfile } from '@/utils/lyricsfile.js'
...
const doSearchLyrics = async () => {
  ...
  try {
    const results = await searchLyrics({
      track_name: title.value,
      artist_name: artistName.value,
      album_name: albumName.value,
    })
    searchResult.value = Array.isArray(results) ? results : results?.data || []
  } catch (error) {
    console.error(error)
    toast.error(error?.data?.message || 'Error searching lyrics')
  } finally {
    loading.value = false
  }
}
...
const apply = async lyricsItem => {
  try {
    ...
    const track = await store.getTrack(props.searchingTrack.id)
    if (track) {
      const syncedLines = lyricsItem.syncedLyrics ? parseLrcLines(lyricsItem.syncedLyrics) : []
      const plainLyrics = lyricsItem.plainLyrics || ''
      const lyricsfile = serializeLyricsfile({
        ...
        plainLyrics,
        syncedLines,
        isInstrumental: !!lyricsItem.instrumental,
      })
      track.lyricsfile = lyricsfile
...
const initialize = async () => {
  ...
  showLineCount.value = config.showLineCount ?? false
}
