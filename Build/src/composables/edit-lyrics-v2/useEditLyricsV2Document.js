...
export function useEditLyricsV2Document({ audioSource, lyricsfile, trackId, progress, toast }) {
  ...
  const saveLyrics = async () => {
    ...
    try {
      ...
      if (isLibraryTrack) {
        // Library track: save to IndexedDB
        const store = useLibraryStore()
        const track = await store.getTrack(trackId.value)
        if (track) {
          track.lyricsfile = serializedContent
          track.lyricsfile_id = lyricsfile?.value?.id ?? null
          await store.updateTrack(track)
        }
      } else {
        // Standalone lyricsfile: save separately
        console.warn('Standalone lyricsfile saving not yet implemented')
      }
