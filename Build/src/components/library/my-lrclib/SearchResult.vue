...
import { searchLyrics, getLyricsById } from '@/api/proxy.js'
...
onMounted(async () => {
  ...
  showLineCount.value = config.show_line_count ?? false
  loading.value = true
  try {
    const results = await searchLyrics({ q: props.keyword })
    tracks.value = Array.isArray(results) ? results : results?.data || []
  } catch (error) {
...
const setEditingTrack = async track => {
  isOpeningTrack.value = true
  try {
    const lyricsData = await getLyricsById(track.id)
    
    const result = {
      lyricsfileId: track.id,
      lyricsfile: lyricsData.content,
      existsInDb: false,
    }
